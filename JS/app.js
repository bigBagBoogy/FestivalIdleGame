...
document.getElementById('getTopPlayers').addEventListener('click', async () => {
    const result = await gameContract.methods.getTopFivePlayers().call();
...
document.getElementById('saveProgress').addEventListener('click', async () => {
    const totalScore = parseInt(prompt('Enter total score:'));
    ...
    try {
        await gameContract.methods.saveProgress(totalScore, podiumLvl, drinksLvl, audioLvl, stageStartOverLvl).send({ from: web3.eth.defaultAccount });
...
document.getElementById('loadProgress').addEventListener('click', async () => {
    try {
        const result = await gameContract.methods.getProgress(web3.eth.defaultAccount).call();
